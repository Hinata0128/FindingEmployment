...
// m_pDx11 メンバーは削除されたため、初期化リストから削除します。
	: m_pDevice11(nullptr)
	...
	, m_pVertexBuffer() // これは配列なので初期化子なし
	, m_pTexture(nullptr)
	, m_pSampleLinear(nullptr)
	, m_Alpha(1.0f)
	, m_Color(1.f, 1.f, 1.f)
	, m_Kerning() // 配列なので初期化子なし
{
}
...
CDebugText::~CDebugText()
{
	...
	// m_pDx11 メンバーは削除されたため、nullptr初期化も不要です。
...
HRESULT CDebugText::Init(CDirectX11& pDx11) // pDx11 は参照として残します
{
...
HRESULT CDebugText::CreateShader()
{
	...
	{
		// _ASSERT_EXPR( false, _T( "hlsl読み込み失敗" ) ); // プロダクションコードではMessageBoxなど別のエラー処理が良い
		return E_FAIL;
	}
