...
//前方宣言.
// CDirectX9とCDirectX11はInit関数で参照として受け取るため、ここでは前方宣言のみ
class CDirectX9;
class CDirectX11;
...
public:
	...
	{
		ALIGN16 D3DXVECTOR4	Diffuse;	//ディフューズ色(拡散反射色).
		ALIGN16 D3DXVECTOR4 Ambient;	//アンビエント色(環境色).
		ALIGN16 D3DXVECTOR4	Specular;	//スペキュラ色(鏡面反射色).
	};
	//フレーム単位で渡す情報.
	// カメラ,ライト情報はここに入れる.
	struct CBUFFER_PER_FRAME
	{
		ALIGN16 D3DXVECTOR4	CameraPos;		//カメラ位置(視点位置).
		ALIGN16 D3DXVECTOR4	vLightDir;		//ライト方向.
	};
...
public:
	...
	// Init関数の引数は、シングルトンインスタンスへの参照として維持
