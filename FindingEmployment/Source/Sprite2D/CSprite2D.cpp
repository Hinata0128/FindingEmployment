...
// WND_W, WND_H の定義が必要な場合はここに追加するか、適切な共通ヘッダに定義があることを確認してください
// 例:
#ifndef WND_W
#define WND_W 1280 // 仮の値、実際のプロジェクトに合わせてください
#endif
#ifndef WND_H
#define WND_H 720  // 仮の値、実際のプロジェクトに合わせてください
#endif

// SAFE_RELEASE マクロの定義 (もし外部で定義されていない場合)
#ifndef SAFE_RELEASE
#define SAFE_RELEASE(x) { if(x) { (x)->Release(); (x)=nullptr; } }
#endif

// _ASSERT_EXPR マクロの定義 (もし外部で定義されていない場合、デバッグビルド用)
#ifdef _DEBUG
#include <crtdbg.h> // _ASSERT_EXPR のために必要
#ifndef _ASSERT_EXPR
#define _ASSERT_EXPR(expr, msg) _ASSERTE(expr && msg) // 簡単な定義、またはプロジェクトの既存マクロを使用
#endif
#else
#ifndef _ASSERT_EXPR
#define _ASSERT_EXPR(expr, msg) ((void)0) // リリースビルドでは何もしない
#endif
#endif
...
CSprite2D::~CSprite2D()
{
	...
	// m_pDx11 メンバーは削除されたため、nullptr初期化も不要
	// m_pContext11 と m_pDevice11 もこのクラスが所有するものではないため、
	// ここでnullptrを代入する必要はありません（Release()で既にnullptr化済み）。
}
...
HRESULT CSprite2D::Init(
	CDirectX11& pDx11, // pDx11 は参照として残します (ただしここでは直接使わない)
	LPCTSTR lpFileName,
	SPRITE_STATE& pSs)
{
	// CDirectX11のシングルトンインスタンスから直接デバイスとコンテキストを取得
...
void CSprite2D::Release()
{
	...
	// デバイスとコンテキストはシングルトンが管理しているので、
	// ここで明示的にnullptrにする必要はありませんが、防御的な意味で残すことは可能です。
	m_pContext11 = nullptr;
	m_pDevice11 = nullptr;
}
